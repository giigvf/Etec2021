...
select p.produto, c.descricao, f.fantasia from categorias c inner join produtos p on (c.id = p.id_categoria)
inner join fornecedor f on (p.id_fornecedor = f.id);

-- exercício --
select n.nome, estoque f.id_forcecedor, vlr.valor_venda from produtos inner join where id_categoria = 1 order by desc;
