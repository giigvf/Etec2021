...
select count(*) itens from produtos;
select min(valor_venda) from produtos;
select max(estoque) from produtos;
select max(estoque) maior_estoque from produtos;
select sum(estoque) from produtos;
select sum(estoque) total_estoque from produtos;
select avg(valor_compra) from produtos;
select avg(valor_compra) valor_medio from produtos;
    
-- exercícios --
select count(id_categoria) from produtos where id_categoria = 2;
select sum(estoque) soma_do_estoque from produtos where id_fornecedor = 1;
select avg(valor_venda and valor_compra) média_valores from produtos where produto = 'ARROZ';
select min(data_nasc) mais_velho from clientes;
select max(data_ultima_venda) from produtos where id_fornecedor = 1 and id_categoria = 1;